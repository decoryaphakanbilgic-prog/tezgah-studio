    ...
    <div className={`relative min-h-screen flex flex-col justify-between selection:bg-amber-200 theme-${theme}`}>
      ...
      <main className="flex-1 bg-[#faf9f6]">
        ...
        {activePage === 'categories' && (
          <motion.div variants={pageContainerVariants} initial="hidden" animate="visible" exit="hidden" className="mx-auto max-w-7xl px-6 py-12 sm:px-8 lg:px-12 space-y-12" id="categories-page">
            <motion.div variants={pageItemVariants} className="text-center md:text-left space-y-2">
              <span className="font-mono text-xs text-gold-600 uppercase tracking-widest block">Ürün Grupları</span>
              <h2 className="font-serif text-3xl sm:text-4xl font-bold tracking-tight text-neutral-900">Tezgah Kategorilerimiz</h2>
              <p className="text-xs sm:text-sm text-stone-500 font-light max-w-2xl">
                Tezgah kategorilerinin fiziki ve yapısal avantajları ile kullanım sınırlarını detaylıca keşfedin.
              </p>
            </motion.div>

            {/* Core Categories Vertical list or Grid detailing Pros/Cons */}
            <div className="space-y-10">
              {MATERIAL_CATEGORIES.map((cat) => (
                <motion.div 
                  key={cat.id}
                  variants={pageItemVariants}
                  className="bg-white border border-stone-200/80 rounded-2xl overflow-hidden shadow-xs grid grid-cols-1 lg:grid-cols-12 gap-0 interactive-card hover-gold-glow"
                  id={`cat-panel-${cat.id}`}
                >
                  {/* Left segment - Summary & Price */}
                  <div className="p-8 lg:col-span-5 bg-stone-warm/50 border-r border-stone-200/60 flex flex-col justify-between">
                    <div className="space-y-4">
                      <span className="font-mono text-[10px] text-gold-600 uppercase tracking-widest font-bold">Kategori Tanımı</span>
                      <h3 className="font-serif text-2xl font-bold text-neutral-950 tracking-wide">{cat.name}</h3>
                      <p className="text-xs text-stone-600 leading-relaxed font-light">{cat.longDescription}</p>
                      
                      <div className="pt-2">
                        <span className="text-[10px] font-bold text-neutral-700 uppercase tracking-wider block">Ortalama Fiyat Skalası</span>
                        <div className="flex items-center space-x-2 mt-1">
                          <span className="font-mono font-bold text-neutral-900 text-sm tracking-widest bg-gold-50/80 border border-gold-200 px-3 py-1 rounded">
                            {cat.averagePrice === 'Lüks' ? '₺₺₺₺₺' : cat.averagePrice === 'Yüksek' ? '₺₺₺₺' : '₺₺₺'}
                          </span>
                          <span className="text-[11px] text-stone-500 font-light">({cat.averagePrice} Segment)</span>
                        </div>
                        <span className="text-[10px] text-stone-400 font-light block mt-1 italic">{cat.priceDetails}</span>
                      </div>
                    </div>

                    <div className="pt-8 space-y-3">
                      <span className="text-[10px] font-bold text-neutral-700 uppercase tracking-wider block">İlişkili Üst Segment Markalar</span>
                      <div className="flex flex-wrap gap-2">
                        {cat.relatedBrands.map((bId) => {
                          const originalBrand = BRANDS.find(brand => brand.id === bId);
                          return (
                            <button
                              key={bId}
                              onClick={() => {
                                if (originalBrand) setSelectedBrand(originalBrand);
                              }}
                              className="text-xs bg-white hover:bg-neutral-950 hover:text-white border border-stone-300 rounded px-3 py-1 transition-colors"
                            >
                              {originalBrand?.name || bId}
                            </button>
                          );
                        })}
                      </div>
                  ...
                  {/* Right segment - Pros, Cons & Suitability */}
                  <div className="p-8 lg:col-span-7 grid grid-cols-1 md:grid-cols-2 gap-6 bg-white shrink-0">
                    <div className="space-y-4">
                      <h4 className="font-serif font-bold text-emerald-800 text-sm flex items-center space-x-1">
                        <Check className="h-4 w-4 stroke-[3px]" />
                        <span>Kusursuz Avantajlar</span>
                      </h4>
                    ...
                    <div className="space-y-4">
                      <h4 className="font-serif font-bold text-amber-800 text-sm flex items-center space-x-1">
                        <X className="h-4 w-4 stroke-[3px]" />
                        <span>Kısıtlar & Hassasiyetler</span>
                      </h4>
                      <ul className="space-y-2 text-xs font-light text-neutral-600 leading-relaxed">
                        {cat.disadvantages.map((dis, i) => (
                          <li key={i} className="flex items-start">
                    ...
                    <div className="md:col-span-2 border-t border-stone-100 pt-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                      <div>
                        <span className="text-[10px] font-bold text-neutral-700 uppercase tracking-wider block mb-1">Mükemmel Uyumlu Alanlar</span>
                        <div className="flex flex-wrap gap-1.5">
                          {cat.usageAreas.map((area) => (
                            <span key={area} className="bg-stone-100 text-stone-700 text-[10px] font-medium px-3 py-1 rounded">
                              {area}
                            </span>
                          ))}
                        </div>
        ...
        {/* 4. COLORS VIEW */}
        {activePage === 'colors' && (
          <motion.div variants={pageContainerVariants} initial="hidden" animate="visible" exit="hidden" className="mx-auto max-w-7xl px-6 py-12 sm:px-8 lg:px-12 space-y-10" id="colors-page">
            <motion.div variants={pageItemVariants} className="text-center md:text-left space-y-2">
              <span className="font-mono text-xs text-gold-600 uppercase tracking-widest block">Ürün Kataloğu</span>
              <h2 className="font-serif text-3xl sm:text-4xl font-bold tracking-tight text-neutral-900">Renk ve Doku Koleksiyonu</h2>
              <p className="text-xs sm:text-sm text-stone-500 font-light max-w-2xl">
                Lüks plakaların yüksek çözünürlüklü dokularını inceleyin, favorilerinizi işaretleyin veya doğrudan teklif listenize ekleyin.
              </p>
            </motion.div>

            {/* Sticky Floating Selected Badges inside page if colors added to quote */}
            {quoteCart.length > 0 && (
              <div className="sticky top-24 z-40 bg-neutral-950 text-white p-4 rounded-2xl flex flex-col sm:flex-row items-center justify-between shadow-lg border border-gold-400 gap-4 transition-all duration-300">
                <div className="flex items-center space-x-3 text-left">
                  <div className="h-10 w-10 bg-gold-500 rounded-full flex items-center justify-center text-neutral-950 font-bold">
                    {quoteCart.length}
                  </div>
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider">Teklif Listenize Eklenen Renkler Mevcut!</h4>
                    <p className="text-[10px] text-stone-300 font-light">Seçtiğiniz lüks numuneler için anında plaka verimlilik raporu alabilirsiniz.</p>
                  </div>
                </div>
                <div className="flex space-x-2 shrink-0">
                  <button
                    onClick={() => setQuoteCart([])}
                    className="text-xs text-neutral-400 hover:text-white px-3 py-2 bg-neutral-900 rounded-lg"
                    id="clear-quote-cart"
                  >
                    Temizle
                  </button>
                  <button
                    onClick={() => {
                      setSelectedCategoryForWizard(null);
                      // Jump to contact fields directly
                      setActivePage('quote-wizard');
                    }}
                    className="text-xs bg-gold-500 hover:bg-gold-600 text-neutral-950 font-bold px-5 py-2.5 rounded-lg uppercase tracking-wider"
                    id="goto-wizard-from-cart"
                  >
                    Teklif Formuna Geç →
                  </button>
                </div>
            ...
            {/* High-Fi Filtration System */}
            <motion.div variants={pageItemVariants} className="bg-white border border-stone-200/60 rounded-2xl p-6 shadow-xs space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                ...
                <div className="space-y-1 md:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      placeholder="Plaka adı veya marka yazın..."
                      value={colorSearch}
                ...
                {/* Color Group Filter */}
                <div className="space-y-1">
                  <label className="block text-[10px] font-bold text-neutral-600 uppercase tracking-wider">Renk Grubu</label>
                  <select
                    value={colorGroupFilter}
                    onChange={(e) => setColorGroupFilter(e.target.value)}
                    className="w-full text-xs py-2 px-3 border border-stone-300 bg-stone-50 rounded-lg outline-none focus:bg-white focus:border-gold-500"
                    id="filter-color-group"
                  >
                    <option value="Tümü">Tümü</option>
                    <option value="Beyaz">Beyaz Tonlar</option>
                    <option value="Gri">Gri Tonlar</option>
                    <option value="Siyah">Siyah Tonlar</option>
                    <option value="Bej">Bej / Sıcak Taş</option>
                  </select>
                </div>

                {/* Effect/Pattern Filter */}
                <div className="space-y-1">
                  <label className="block text-[10px] font-bold text-neutral-600 uppercase tracking-wider">Efekt / Doku</label>
                  <select
                    value={colorEffectFilter}
                    onChange={(e) => setColorEffectFilter(e.target.value)}
                    className="w-full text-xs py-2 px-3 border border-stone-300 bg-stone-50 rounded-lg outline-none focus:bg-white focus:border-gold-500"
                    id="filter-effect"
                  >
                    <option value="Tümü">Tümü</option>
                    <option value="Damarlı">Damarlı (Calacatta/Statuario)</option>
                    <option value="Düz">Düz Pürüzsüz</option>
                    <option value="Beton">Mermer / Beton Görünümü</option>
                    <option value="Traverten">Traverten Dokulu</option>
                    <option value="Granit">Granit / Noktalı Dokular</option>
                  </select>
                </div>
              </div>

              {/* Sub-filters in row chips */}
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-t border-stone-100 pt-4 text-xs font-light text-stone-500">
                <div className="flex flex-wrap items-center gap-3">
                  <span className="font-bold text-[10px] text-neutral-600 uppercase tracking-wider">Bitiş Yüzeyi:</span>
                  <div className="flex space-x-1.5">
                    {['Tümü', 'Mat', 'Parlak', 'İpeksi'].map((fin) => (
                      <button
                        key={fin}
                        onClick={() => setColorFinishFilter(fin)}
                        className={`px-3 py-1 text-[11px] rounded-lg border transition-colors ${
                          colorFinishFilter === fin
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <span className="font-bold text-[10px] text-neutral-600 uppercase tracking-wider">Kullanım Koşulu:</span>
                  <div className="flex space-x-1.5">
                    {['Tümü', 'İç mekan', 'Dış mekan', 'Yoğun Kullanım'].map((suit) => (
                      <button
                        key={suit}
                        onClick={() => setColorSuitabilityFilter(suit)}
                        className={`px-3 py-1 text-[11px] rounded-lg border transition-colors ${
                          colorSuitabilityFilter === suit
                            ? 'bg-neutral-900 border-neutral-900 text-white'
                            : 'bg-stone-50 border-stone-200 text-stone-600 hover:border-stone-400'
                        }`}
                        id={`btn-suit-${suit.replace(/\s/g, '')}`}
                      >
                        {suit}
                      </button>
                    ))}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {filteredColors.map((color) => {
                const isFavorite = favorites.includes(color.id);
                const isInCart = quoteCart.includes(color.id);
                
                return (
                  <motion.div
                    key={color.id}
                    variants={pageItemVariants}
                    className="group bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-xs hover:shadow-md transition-luxury flex flex-col justify-between interactive-card hover-gold-glow"
                    id={`color-grid-card-${color.id}`}
                  >
                    {/* Visual Frame */}
                    <div className="relative h-60 w-full overflow-hidden bg-stone-100">
                      ...
                      {/* Top left Brand pill status */}
                      <span className="absolute top-3 left-3 bg-neutral-950/80 backdrop-blur-md text-white text-[9px] font-bold px-2.5 py-1 rounded-full border border-white/10 uppercase tracking-wider shadow-sm">
                        {color.brand}
                      </span>

                      {/* Top right favorite option */}
                      <button
                        onClick={() => handleToggleFavorite(color.id)}
                        className={`absolute top-3 right-3 h-8 w-8 rounded-full border transition-all flex items-center justify-center bg-white/90 backdrop-blur-sm shadow-sm ${
                          isFavorite 
                            ? 'text-red-500 border-red-200 bg-red-50/25' 
                            : 'text-stone-600 border-stone-200 hover:text-red-500'
                        }`}
                        id={`btn-fav-color-${color.id}`}
                      >
                      ...
                      <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-white">
                        <span className="text-[10px] font-mono tracking-widest uppercase bg-neutral-950/60 px-2 py-0.5 rounded backdrop-blur-xs">
                          {color.materialType}
                        </span>
                        <span className="text-[10px] text-stone-200 font-light truncate max-w-[120px]">
                          {color.finish} Yüzey
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="p-4 space-y-3">
                      <div>
                        <h4 className="font-bold text-neutral-900 text-sm tracking-wide group-hover:text-gold-600 transition-colors">
                          {color.name}
                        </h4>
                        <div className="flex space-x-1.5 mt-1.5">
                          {color.suitability.map((suit) => (
                            <span key={suit} className="text-[8px] font-mono uppercase bg-stone-100 text-stone-500 rounded px-1.5 py-0.5 font-semibold">
                              {suit}
                            </span>
                          ))}
                        </div>
                      ...
                      {/* Suitability details */}
                      <div className="text-[10px] text-neutral-500 font-light leading-snug">
                        Doku deseni: <strong className="text-neutral-700">{color.effect}</strong>
                      </div>
                    </div>

                    {/* CTA selection area */}
                    <div className="border-t border-stone-100 p-3 bg-stone-warm flex space-x-2">
                      <button
                        onClick={() => handleToggleQuoteCart(color.id)}
                        className={`flex-1 py-1.5 py-2 text-[10px] font-semibold tracking-wider rounded-lg uppercase transition-all duration-300 flex items-center justify-center space-x-1 ${
                          isInCart 
                            ? 'bg-emerald-600 text-white hover:bg-emerald-700' 
                            : 'bg-neutral-950 text-stone-200 hover:bg-gold-600 hover:text-white'
                        }`}
                        id={`btn-add-cart-${color.id}`}
                      >
                        {isInCart ? (
                          <>
                            <Check className="h-3.5 w-3.5 shrink-0" />
                            <span>Teklife Eklendi</span>
                          </>
                        ) : (
                          <>
            ...
            {filteredColors.length === 0 && (
              <div className="text-center py-16 bg-white border border-stone-200/60 rounded-2xl space-y-2">
                <AlertCircle className="h-8 w-8 text-stone-400 mx-auto" />
                <h4 className="text-sm font-semibold text-neutral-800">Uyumlu Renk Karosu Bulunamadı</h4>
                <p className="text-xs text-stone-500 max-w-sm mx-auto font-light">Lütfen filtre kısıtlamalarını temizleyerek geniş aramaya dönünüz.</p>
                <button
                  onClick={() => {
                    setColorSearch('');
                    setColorGroupFilter('Tümü');
                    setColorEffectFilter('Tümü');
                    setColorFinishFilter('Tümü');
                    setColorSuitabilityFilter('Tümü');
                  }}
                  className="rounded-full bg-neutral-950 px-5 py-2 text-[10px] tracking-widest text-white uppercase font-semibold mt-3"
                  id="reset-color-filters"
                >
                  Filtreleri Sıfırla
                </button>
              </div>
